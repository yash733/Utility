...
class vector_db:
    if '_vectore_db' not in st.session_state: 
        st.session_state._vector_db = None

    @classmethod
    def create_vector_db(cls):
        if st.session_state._vector_db == None:
            embedding = OllamaEmbeddings(model="nomic-embed-text:v1.5")
            reduced_document = data_extraction.data_flatning()
            # vectorstore will hold important data @files
            st.session_state._vector_db = FAISS.from_documents(documents=reduced_document, embedding=embedding)
    ...
    def get_retrieval_chain(cls, model, prompt):
        ...
        retriever = st.session_state._vector_db.as_retriever(search_kwargs={'k': 20}, 
...
class agents:
    ...
    def resume_maker(cls, state: State):
        ...
        if st.session_state.state == 'START': # State tracking
            ...
            log.info(f"Prompt 1 input --> {input_data}") # log
        ...
        # === Loop RUN === #
            ...
            log.debug(f'Resume to Improve on - {state.get("resume")}')
            ...
            log.info(f"Prompt2 input --> {input_data}")
        ...
        # === Loop RUN === #
            ...
            input_data = {'input' : state.get('suggestion'),
                          'resume' : state.get('resume'),
                          'chat_history' : trimmed_history}
            log.info(f"Prompt3 input --> {input_data}")
        ...
        log.info(f"Meta Data - {result.get('answer').meta_data}") # log
        ...
        log.debug(f"State UPDATED ? {state}") # log
    ...
    def review_agent(cls, state: State):
        ...
        log.info(f"Rewiew input --> {input_data}") # log
        ...
        log.critical(f"Review Agent output --> {output}") # log
        log.debug(f"State UDATED --> {state}") # log
        if output.sentiment == 'Perfect':
            log.info(f'Review Agent - Final Resume == {output.resume}') # log
            return {'final_resume': output.resume}
            
        elif output.sentiment == 'Improvement Required':
            log.info(f'Review Agent - Improvent is required \n\nsuggestion: {output.suggestion} \n\nsentiment: {output.sentiment} \n\nresume: {output.resume}') # log
    ...
    def sentiment(cls, state: State):
        ...
        if st.session_state.output_data.get('Re_rout_') == True:
    ...
    def resume_graph(cls, state):
        _graph = (
            ...
            .add_conditional_edges('User Feedback', cls.sentiment, {'END': END, 'Perfect': 'Review Expert', 'Improvement Required': 'Resume Maker'})
